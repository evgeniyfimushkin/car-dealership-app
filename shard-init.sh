...
# Инициализация репликации для Shard 4
echo "Инициализация репликации для Shard 4..."
docker exec -it shard4 mongosh --port 27018 --eval "
rs.initiate({
  _id: \"$REPL_SET_SHARD4\",
  members: [{ _id: 0, host: \"$SHARD4\" }]
});"
...
sh.addShard(\"$REPL_SET_SHARD4/$SHARD4\");
"
...
export MONGO_URL=mongodb://localhost:27017/car-dealership